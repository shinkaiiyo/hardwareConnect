...
typedef struct
{
    uint8_t rcv_buf[MAX_PROTOBUF_LENGTH];

    uint8_t temp_buf;
    uint8_t rcv_state;
    uint8_t rcv_len;
    uint8_t rcv_frame_len;

    uint8_t timeout_flag;
    uint32_t time_counter;
    uint8_t RECEIVE_PROTO_FLAG;
}recvUartDataTypedef;
...
public slots:
    ...
    bool tryConnect(QString portName, quint32 baudrate , quint32 dataBits, quint32 parity, quint32 stopBits);
    void tryDisconnect();
